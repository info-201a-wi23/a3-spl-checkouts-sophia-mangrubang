...
# create a dataset of total checkouts for each book in the series (audiobooks)
...
ggplot(audio_checkouts_barchart) + geom_col(aes(x = short_name, y = checkout_count, fill = Title )) + labs(title = "All Audiobooks in the Series and Their Total Checkouts in 2022-2023", x = "Book", y = "Number of Checkouts" ) + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) + scale_y_continuous(breaks = seq(0, 250, 10)) + scale_fill_discrete(name = "Title of Audiobook")
